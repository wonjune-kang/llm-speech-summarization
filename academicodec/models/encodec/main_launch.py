...
from academicodec.models.encodec.loss import criterion_d, criterion_g, loss_dis, loss_g
...
from academicodec.models.soundstream.models import (
    MultiPeriodDiscriminator,
    MultiScaleDiscriminator,
)
from academicodec.utils import Logger, seed_everything
...
def get_args():
    ...
    # args for random
    parser.add_argument(
        "--seed", type=int, default=6666, help="seed for initializing training. "
    )
    parser.add_argument(
        "--cudnn_deterministic",
        action="store_true",
...
def main_worker(local_rank, args):
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.BATCH_SIZE, num_workers=8, sampler=train_sampler
    )
    valid_loader = torch.utils.data.DataLoader(
        valid_dataset, batch_size=args.BATCH_SIZE, num_workers=8, sampler=valid_sampler
    )
